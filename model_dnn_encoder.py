...
def get_default_hparams():
  ...
  hparams = HParams(
      data_set=['sketchrnn_cat.npz', 'sketchrnn_bus.npz', 'sketchrnn_dog.npz','sketchrnn_rabbit.npz','sketchrnn_tree.npz'],  # Our dataset. ,, 'sketchrnn_tree.npz', 'sketchrnn_dog.npz'
      ...
      z_size=128,  # Size of latent vector z. Recommend 32, 64 or 128. 288
      ...
      learning_rate=0.00001,  # Learning rate. 0.001
      decay_rate=0.99999,  # Learning rate decay per minibatch. 0.9999
      kl_decay_rate=0.99995,  # KL annealing decay rate per minibatch.
      min_learning_rate=0.0000001,  # Minimum learning rate. 0.00001
...
class Model(object):
  ...
  def dnn_encoder(self, x):
    #print ("x", x)
    fc1 = tf.layers.dense(x, 200, name="fc1")#1000 200 256
    #print ("fc1, ", fc1)
    fc2 = tf.layers.dense(fc1, 100, name="fc2")#500 100 128
    #print ("fc2, ", fc2)
    fc3 = tf.layers.dense(fc2, 50, name="fc3")#250 50 64
    #print ("fc3, ", fc3)
    fc4 = tf.layers.dense(fc3, 25, name="fc4")#100 25 32
    #print ("fc4, ", fc4)   

    mu = tf.layers.dense(fc4, self.hps.z_size, name="fc_mu")
    print ("mu, ", mu)
    presig = tf.layers.dense(fc4, self.hps.z_size, name="fc_sigma")
    print ("presig, ", presig)
    return mu, presig

  def dnn_encoder_dropout(self, x):
    #print ("x", x)
  ...
  def build_model(self, hps):
    ...
    self.onv_data_left = tf.placeholder(tf.float32, [self.hps.batch_size, self.hps.onv_size])
    self.onv_data_right = tf.placeholder(tf.float32, [self.hps.batch_size, self.hps.onv_size])

    # either do vae-bit and get z, or do unconditional, decoder-only
    if hps.conditional:  # vae mode:
      #left eye
      with tf.variable_scope("left"):
        self.last_h_left = self.dnn_encoder_dropout(self.onv_data_left)
      #right eye
      with tf.variable_scope("right"):
        self.last_h_right = self.dnn_encoder_dropout(self.onv_data_right)

      #concat the result of two eyes
      self.last_h = tf.concat([self.last_h_left,self.last_h_right],1)
      print (self.last_h)

      self.mean = tf.layers.dense(self.last_h, self.hps.z_size, name="fc_mu")
      print ("mu, ", self.mean)
      self.presig = tf.layers.dense(self.last_h, self.hps.z_size, name="fc_sigma")
      print ("presig, ", self.presig)
