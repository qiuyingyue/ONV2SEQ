...
def conv_relu_layer_bn(name, bottom, phase_bn, kernel_size, stride, output_dim, padding='SAME',
                    bias_term=True, weights_initializer=None, biases_initializer=None):
    conv = conv_layer(name, bottom, kernel_size, stride, output_dim, padding,
                      bias_term, weights_initializer, biases_initializer)
#    conv = tf.nn.batch_norm(conv, center=True, scale=True, is_training=phase_bn, scope='bn') 
    with tf.variable_scope(name):
        conv = tf.contrib.layers.batch_norm(conv, center=True, scale=True, is_training=phase_bn, scope='bn') 
    relu = tf.nn.relu(conv)
    return relu
